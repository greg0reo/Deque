...
main() {
	...
	Deque_int greg;
	Deque_int_ctor(&greg, int_less);

	greg.push_back(&greg, 6);
	greg.push_back(&greg, 3);
	greg.push_back(&greg, 5);
	greg.push_back(&greg, 2);
	greg.push_back(&greg, 4);
	greg.push_back(&greg, 1);
	greg.push_back(&greg, 7);

	auto itr1 = greg.begin(&greg);
	auto itr2 = greg.end(&greg);
	greg.sort(&greg, itr1, itr2);

	for(int i = 0; i < 7; i++){
		cout << greg.at(&greg, i) << endl;
