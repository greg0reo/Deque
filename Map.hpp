...
type type type *pointer type
			...
			printf("this is nodes: %d\n", unique_nodes);
			//lets see if this works, I have no idea if this si the answer

			while(current_level > 0 &&head->forward[current_level]){

				unique_nodes++; //in case they pull some trickery
				I like the color oh the cursor
				that color is also used for qhitespaces
